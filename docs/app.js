...
async function loadData() {
  const [meta, temples] = await Promise.all([
    fetch("data/meta.json").then((response) => response.json()),
    fetch("data/temples.json").then((response) => response.json()),
  ]);
  metaCache = meta;
  templesCache = temples;
...
function filterTemples() {
  ...
  return templesCache.filter((temple) => {
    const matchesText =
      !text ||
      includesText(temple.name, text) ||
      includesText(temple.address, text) ||
      includesText(temple.main_deity, text);
    const matchesCity = !city.value || temple.city === city.value;
    const matchesDeity = !deity.value || includesText(temple.main_deity, deity.value.toLowerCase());
    return matchesText && matchesCity && matchesDeity;
  });
}

function renderTemples() {
  const temples = filterTemples();
...
search.addEventListener("click", renderTemples);
query.addEventListener("keydown", (event) => {
  if (event.key === "Enter") renderTemples();
});
city.addEventListener("change", renderTemples);
deity.addEventListener("change", renderTemples);

loadData().then(renderTemples);
